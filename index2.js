...
authors.forEach(author => {
    const url = `https://dtruyen.com/tac-gia/${slug(author)}/`
    request(url, (error, response, html) => {
        if(!error && response.statusCode == 200) {
            const $ = cheerio.load(html); // load HTML
            

            $('.thumb img').each((index, el) => { 
            const image = $(el).attr('data-layzr');
            const image_replace = image.replace("small", "large");
            const title = $(el).attr('alt');
            const file_name = slug(title)
            imgs.push({
                uri: image_replace,
                filename: `${file_name}`
            })
            ...
            imageDownloader({
                imgs: imgs,
                dest: './Cổ Long', //destination folder
            })
                .then((info) => {
